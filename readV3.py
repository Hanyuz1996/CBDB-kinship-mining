    ...
    while(not Q.empty()):
        ...
        for key,value in kin_temp1.iterrows():
            ...
            if value[1] in allkindatacode:
                if len(kindatacode.loc[value[1],'c_kinrel'])<=10:
                    if(value[0] in unvisited):
                        Q.put(value[0])
                        Q0.put(value[0])
                        if set("WMCHAP") & set(kindatacode.loc[value[1],'c_kinrel'])!=set():
                            #if abs(kindatacode.loc[value[1],'代差'])>5:
                            #    gen[value[0]]=gen[k]+kindatacode.loc[value[1],'代差']/abs(kindatacode.loc[value[1],'代差'])*5
                            #else:
                        ...
                        if set("WMHCAP") & set(kindatacode.loc[value[1],'c_kinrel'])!=set():
                            if gen[value[0]]!=-1000:
                                if gen[k]+kindatacode.loc[value[1],"代差"]!=gen[value[0]]:
                                    ...
                                    crash = crash + 1
                                #if abs(kindatacode.loc[value[1],'代差'])>5:
                                #    p=gen[k]+kindatacode.loc[value[1],'代差']/abs(kindatacode.loc[value[1],'代差'])*5
                                #else:
...
Nodes = sorted(TMZ.nodes(),key=genn)
i = 0
for j in range(0,maxdepth+1):
    GraphVizPrint.append("{")
    #GraphVizPrint.append('rank="same";')
    while gen[Nodes[i]]==j:
        GraphVizPrint.append('%d[label="(%s)"];'% (Nodes[i],biogmain.loc[Nodes[i],"c_name_chn"]))
        i = i + 1
        if i==len(Nodes):
            break
    GraphVizPrint.append("}")
